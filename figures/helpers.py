...
def get_prepared_pdf(pdf_data, logo_url):
    ...
    class LearnerPDF(FPDF):
        ...
        def header(self):
            ...
            self.ln(1)
            img_w, img_h = self.get_image_dimensions(logo_url)
            x_position = (self.w - img_w) / 2
            self.image(name=logo_url, x=x_position, y=self.get_y(), w=img_w, h=img_h)
            self.ln(5)
...
def send_insights_course_detail_report(
    ...
    for learner in learners:
        csv_report_writer.writerow([
            ...
            '{}%'.format(learner['courses'][0]['progress_data']['percent_grade']),
