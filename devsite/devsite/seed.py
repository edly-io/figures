...
FAKE = faker.Faker()
LAST_DAY = days_from(datetime.datetime.now(), -2).replace(tzinfo=datetime.timezone.utc)
...
def seed_course_overviews(data=None):
    ...
    for rec in data:
        ...
        defaults = dict(
                ...
                number=rec['number'],
                created=as_datetime(rec['created']).replace(tzinfo=datetime.timezone.utc),
                start=as_datetime(rec['enrollment_start']).replace(tzinfo=datetime.timezone.utc),
                end=as_datetime(rec['enrollment_end']).replace(tzinfo=datetime.timezone.utc),
                enrollment_start=as_datetime(rec['enrollment_start']).replace(tzinfo=datetime.timezone.utc),
                enrollment_end=as_datetime(rec['enrollment_end']).replace(tzinfo=datetime.timezone.utc),
            )
    ...
    for rec in data:
        try:
            ...
            user.date_joined = as_datetime(
                FAKE.date_between(first_date, LAST_DAY)).replace(tzinfo=datetime.timezone.utc)
            user.save()
...
def seed_course_enrollments_for_course(course_id, users, max_days_back):
    ...
    for user in users:
        ...
        CourseEnrollment.objects.update_or_create(
            course_id=course_id,
            user=user,
            created=as_datetime(enroll_date(max_days_back)).replace(tzinfo=datetime.timezone.utc),
            )
        ...
        for ce in qs.order_by('?')[:sample]:
            GeneratedCertificate.objects.create(
                ...
                created_date=as_datetime(FAKE.date_between(
                    ce.created, LAST_DAY)).replace(tzinfo=datetime.timezone.utc),
            )
